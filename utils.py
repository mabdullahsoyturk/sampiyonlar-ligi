...
def export_to_csv(fund_type, best_funds):
    with open(fund_type + ".csv", "w") as fund_file:
        ...
        for fund in best_funds:
            row = ",".join([fund.code,fund.name,fund.type,fund.one_month,fund.three_months,fund.six_months,fund.from_new_year,fund.one_year,fund.three_years,fund.five_years])
            fund_file.write(row + "\n")